import { Dealflow, Filter, formatInterval } from "@dealbase/core";
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    const { dealflow, filter } = JSON.parse(req.body) as {
      dealflow: Dealflow;
      filter: Filter;
    };
    ...
    cloudinary.api.resources_by_context(
      "country",
      filter.country[0] === "Africa" || filter.country.length > 1
        ? "Africa"
        : filter.country[0].toLowerCase().replace(" ", "-"),
