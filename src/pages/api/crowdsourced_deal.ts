...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    if (req.method === "POST") {
      ...
      // const data = await db.create(DatabaseTables.CrowdsourcedDeals, body);

      logger.info("Finished Saving Crowdsouced Deal", loggerMetadata);

      res.status(200).json({ success: true, crowdsourced_deal: {} });
    }
    ...
    if (req.method === "PATCH") {
      ...
      //   DatabaseTables.CrowdsourcedDeals,
      //   updateData
      // );

      logger.info("Finished Updating Crowdsourced Deal", loggerMetadata);

      res.status(200).json({ crowdsourced_deal: {} });
    }
    ...
    if (req.method === "DELETE") {
      ...
      // const data = await db.delete(DatabaseTables.CrowdsourcedDeals, id);
    ...
    if (req.method === "GET") {
      ...
      const crowdsourced_deals = await prisma.crowdsourcedDeal.findMany();
      // const { data: crowdsourced_deals, error } = await supabase
      //   .from(DatabaseTables.CrowdsourcedDeals)
      //   .select("*");
