...
import { Deal, TableInstanceWithHooks } from "@dealbase/core";
...
export const CompanyPopoverDealsTable = ({ companyId, dealId }: Props) => {
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        Header: "Investors",
        accessor: (row: Deal) =>
          (row.investors.map((r) => r.investor.name) as string[]).join(", "),
        id: "investors",
      },
      {
  ...
  return (
    ...
    >
      ...
      <Grid
        ...
        gap={2}
      >
        {deals.length === 0 && <Text>{`No other deals found`}</Text>}
