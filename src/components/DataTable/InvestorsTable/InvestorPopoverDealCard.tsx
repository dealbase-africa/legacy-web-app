    ...
    >
      <Flex gap={2}>
        {deal.company.logo?.cloudinaryPublicId && (
          <CloudinaryImage
            publicId={deal.company.logo.cloudinaryPublicId}
            display="flex"
            alignItems="center"
      ...
      {deal.pressRelease?.link ? (
        <LinkText
          href={deal.pressRelease?.link}
        ...
        <Text>
          {deal.pressRelease?.date
            ? new Date(deal.pressRelease?.date).toLocaleDateString()
            : "Undisclosed"}
        </Text>
      )}
    </GridItem>
